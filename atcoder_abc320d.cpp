...
void dfs(int node,vector<array<int,3>> graph[],vector<pair<ll,ll>> &ans,vector<int> &vis)
{
    vis[node]=1;
    for(auto [child,x,y] : graph[node])
    {
        if(vis[child]==1) continue;
        ans[child]={ans[node].ff+x,ans[node].ss+y};
        dfs(child,graph,ans,vis);
    }
}
...
void solve()
{
    ...
    vector<pair<ll,ll> > ans(n+1,{INF,INF});
    ...
    for(int i=1;i<=n;i++)
    {
        if(ans[i].ff==INF) cout<<"undecidable"<<el;
        else cout<<ans[i].ff<<sp<<ans[i].ss<<el;
    }
